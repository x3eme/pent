...
class Data:
    ...
    def get_5min_by_symbol(self,symbol) -> pandas.DataFrame:
        ...
        with warnings.catch_warnings():
            ...
            newdf.drop(newdf.tail(len(newdf)-252).index, inplace=True)
        # print(str(len(newdf)))
    ...
    def update(self) -> pandas.DataFrame:
        ...
        try:
            conn = self.dbconnect()
            cursor = conn.cursor()
            postgreSQL_select_Query = "SELECT t1,s3, o7 as Open,h9 as High,l10 as Low,c8 as Close,v11 as Volume FROM klines order by t1 desc LIMIT 35360 " #WHERE x13='true'
            cursor.execute(postgreSQL_select_Query)
            kline_records = cursor.fetchall()
            df = pandas.DataFrame(kline_records)
            # df= df.reindex(index=df.index[::-1])
