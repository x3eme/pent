...
class Backtest:
    def __init__(self):
        self.pairs=["ADAUSDT","ETHUSDT", "BTCUSDT"]
        self.period = 1 #period in months
        self.start_date = datetime.datetime(2020, 7, 1)
    ...
    def start(self, pair):
        ...
        self.finish_timespan = self.start_timespan+(self.period * 2 * 24 * 12 * 5 * 60000)
        ...
        self.cur.execute("SELECT timespan1, CAST(open as float) as open ,CAST(high as float) as high ,CAST(low as float) as low ,CAST(close as float) as close,volume FROM "+pair+" where timespan1>"+str(self.start_timespan-1)+" and timespan1<"+str(self.finish_timespan+1)+"")
        ...
        while self.go:
            ...
            if((ind+22)>total_rows):
                ...
                ind += 1
        print(datetime.datetime.now())
        self.pl.plot(ex1.closed_positions, pair, self.start_date)
